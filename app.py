...
def notificar_vencimentos():
    conn = None
    try:
        # Inicializa o cliente instanciando as chaves do .env
        ...
        # Query de negócio: seleciona treinamentos que vencem nos próximos 30 dias
        sql = """
        ...
        contador_sucesso = 0
        for reg in vencendo:
            # Aplica a limpeza de string do telefone obtido do banco
            telefone_limpo = formatar_para_twilio(reg['TELEFONE_FUN'])
            
            # Validação defensiva: se o número estiver incompleto ou nulo, o loop pula o registro
            ...
            # Estruturação do texto de aviso de SST
            mensagem = f"Olá {reg['NOME_FUN']}, seu treinamento de {reg['NOME_NR']} vence em breve no dia {reg['DATA_VENCIMENTO']}."
            
            # 1. Envio de SMS Comercial
            ...
            # 2. Envio de WhatsApp Corporativo
            client.messages.create(
                body=mensagem,
...
class Adm:
    def __init__(self, id_adm, nome_adm, email_adm, cpf_adm, telefone_adm, cargo_adm, setor_adm, whatsapp_adm):
        self.id_adm = id_adm
        ...
        self.whatsapp_adm = whatsapp_adm


class Funcionarios:
    def __init__(self, id_fun, nome_fun, email_fun, cpf_fun, telefone_fun, cargo_fun, setor_fun, whatsapp_fun):
        self.id_fun = id_fun
        ...
        self.whatsapp_fun = whatsapp_fun
...
def cadastrar_adm():
    try:
        ...
        sql = """
        INSERT INTO adm (NOME_ADM, EMAIL_ADM, CPF_ADM, TELEFONE_ADM, CARGO_ADM, SETOR_ADM, WHATSAPP_ADM)
...
def cadastrar_fun():
    try:
        ...
        sql = """
        INSERT INTO funcionarios (NOME_FUN, EMAIL_FUN, CPF_FUN, TELEFONE_FUN, CARGO_FUN, SETOR_FUN, WHATSAPP_FUN)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
...
def atualizar_relatorio(id_rel):
    try:
        ...
        sql_check = "SELECT * FROM relatorio WHERE ID_REL = %s"
        cursor.execute(sql_check, (id_rel,))
        existente = cursor.fetchone()
...
#DELETAR RELATÓRIO
@app.route('/api/relatorio/<id_rel>', methods=['DELETE'])
def deletar_rel(id_rel):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        sql = "DELETE FROM relatorio WHERE ID_REL = %s"
        cursor.execute(sql, (id_rel,))
        conn.commit()
